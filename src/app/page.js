            ...
            <div className="relative z-10 mx-auto max-w-[1440px] px-4 sm:px-8 py-32 sm:py-48">
              <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-10">
                <div className="max-w-xl">
                  <h1 className="text-4xl sm:text-6xl font-bold leading-tight text-white text-balance">
                  ...
                  <form onSubmit={submitHeroSearch} className="mt-4 flex overflow-hidden rounded-full bg-white shadow-lg">
                    <input
                      value={draft}
                ...
                <div className="w-full max-w-xs shrink-0 rounded-2xl border border-white/20 bg-slate-900/60 p-5 backdrop-blur-md lg:mt-2">
                    ...
                    {mode === "hire"
                      ? topAgents.map((a) => (
                          <Link
                            key={a.id}
                            href={`/agents/${a.id}`}
                            className="flex items-center gap-2 rounded-lg bg-white/10 px-3 py-2.5 text-sm text-white hover:bg-white/20 transition"
                          >
                            <span className="grayscale">{a.emoji}</span>
                            <span className="flex-1 truncate font-semibold">{a.name.split(" · ")[0]}</span>
                            <span className="shrink-0 text-white/60">완료 {a.completedJobs}건</span>
                          </Link>
                        ))
                      : openJobs.map((j) => (
                          <Link
